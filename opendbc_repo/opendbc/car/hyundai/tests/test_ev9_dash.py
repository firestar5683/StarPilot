...
def test_existing_canfd_speed_limit_helper_keeps_stock_units_and_range():
  CP = SimpleNamespace(flags=int(HyundaiFlags.CANFD_LKA_STEERING))
  FPCP = SimpleNamespace(flags=int(HyundaiStarPilotFlags.SPEED_LIMIT_AVAILABLE))
  values = {"ISLW_SpdCluMainDis": 60, "ISLA_SpdWrn": 1}
  cp = SimpleNamespace(vl={"FR_CMR_02_100ms": values})
  cp_cam = SimpleNamespace(vl={"FR_CMR_02_100ms": values})

  assert calculate_canfd_speed_limit(CP, FPCP, cp, cp_cam, 0.5) == 30.0
  values["ISLW_SpdCluMainDis"] = 253
  assert calculate_canfd_speed_limit(CP, FPCP, cp, cp_cam, 0.5) == 0.0
