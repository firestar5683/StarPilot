...
def test_mrr35_display_discriminator_decodes_on_all_tracks():
  ...
  for addr in range(MRR35_RADAR_START_ADDR, MRR35_RADAR_START_ADDR + MRR35_RADAR_MSG_COUNT):
    ...
    msg = packer.make_can_msg(name, 0, {"NEW_SIGNAL_7": 321})
    parser.update([(1, [msg])])
    assert parser.vl[name]["NEW_SIGNAL_7"] == 321
    assert ev9_mrr35_cluster_display_candidate(parser.vl[name])
