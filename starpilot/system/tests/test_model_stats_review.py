...
def annotated_fixture(tmp, pair=False):
    tel, reducer = Telemetry(), Reducer()
    owner = PAIR if pair else OWNER
    reducer.update(frame(tel, 1.0, owner=owner))
    reducer.update(frame(tel, 1.1, owner=owner))
    if not pair:
        reducer.update(frame(tel, 2.0, owner=''))
    db = Path(tmp) / 'stats.sqlite'
    store = Store(db)
    try:
        store.checkpoint('synthetic-drive', 1000, reducer.snapshot(), now=1001)
        with patch('openpilot.starpilot.common.model_stats_store.time.time', return_value=1002):
            api = ModelStatsAPI(db)
            model = api.annotate([{'value': 'synthetic-a', 'builtin': False}], tmp, Path(tmp) / 'missing-builtin', {}, lambda key: key + '.pkl')[0]
            return model, api.summary()
    finally:
        store.close()

def frontend_case(pair):
    with tempfile.TemporaryDirectory(prefix='synthetic-ui-', dir=OUT) as tmp:
        model, summary = annotated_fixture(tmp, pair)
    result = subprocess.run(['node', str(ROOT / 'starpilot/system/the_galaxy/tests/tracking_status_probe.mjs')], input=json.dumps(model), text=True, capture_output=True, timeout=10, check=True)
    rendered = json.loads(result.stdout)
    save('frontend-pair' if pair else 'frontend-unavailable', {'apiSummary': summary, 'annotatedModel': model, 'rendered': rendered})
    return model, rendered

def test_frontend_preserves_telemetry_unavailable_status():
    model, rendered = frontend_case(False)
    assert model['stats']['trackingStatus'] == 'telemetry_unavailable'
    assert 'unavailable' in rendered['trackingStatus'].lower()

def test_frontend_pair_use_is_not_not_driven():
    model, rendered = frontend_case(True)
    assert model['stats']['usedInPairs'] is True
    assert rendered['trackingStatus'] != 'Not driven yet'
