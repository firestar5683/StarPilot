...
def setup(app):
  ...
  def engine_release():
    with Panda(disable_checks=True) as panda:
      panda.can_send(0x7E1, VOLT_ENGINE_RELEASE_CMD, 0, timeout=0)
      time.sleep(0.35)
      panda.can_send(0x7E1, VOLT_ENGINE_SEND, 0, timeout=0)
  ...
  class EngineOnThread(StoppableThread):
    def run(self):
      while self.running:
        with Panda(disable_checks=True) as panda:
          panda.can_send(0x7E1, VOLT_ENGINE_ON_CMD, 0, timeout=0)
          time.sleep(0.35)
          panda.can_send(0x7E1, VOLT_ENGINE_SEND, 0, timeout=0)
        time.sleep(2)
  ...
  class EngineOffThread(StoppableThread):
    def run(self):
      while self.running:
        with Panda(disable_checks=True) as panda:
          panda.can_send(0x7E1, VOLT_ENGINE_OFF_CMD, 0, timeout=0)
          time.sleep(0.35)
          panda.can_send(0x7E1, VOLT_ENGINE_SEND, 0, timeout=0)
        time.sleep(2)
