...
from openpilot.starpilot.common.longitudinal_mode_actions import LEGACY_MODE_ACTIONS, ACTION_LABELS
...
class WheelControlsDaemon:
  def __init__(self, params: Params | None = None, params_memory: Params | None = None):
    ...
    from openpilot.starpilot.common.action_feedback import ActionFeedback
    self.feedback = ActionFeedback(self.params, self.params_memory)
  ...
  def _handle_key(self, source: InputSource, code: int) -> None:
    ...
    for mapping in mappings:
      if mapping["device_id"] == source.device_id and mapping["event_code"] == code:
        try:
          self.feedback.execute(mapping["slot"], execute_controller_key)
        except Exception:
  ...
  def _publish_status(self, now: float) -> None:
    ...
    status = {
      "last_action": self.feedback.receipt,
      "updated_at": now,
  ...
  def run(self) -> None:
    try:
      while True:
        ...
        if now - self.last_status >= STATUS_INTERVAL_SECONDS or self.feedback.receipt is not getattr(self, "_published_receipt", None):
          self._publish_status(now)
          self.last_status = now
          self._published_receipt = self.feedback.receipt
    finally:
      self.close()
