...
const mode = '__starpilot_favorite_action__:longitudinal_cycle'
...
test('classic longitudinal action uses fresh acknowledged snapshot and expiry', async () => {
  const {context, requests, fetch, snapshot} = setup()
  const previous = globalThis.fetch
  try {
    globalThis.fetch = fetch
    await context.activateFavoriteAction(mode)
    assert.equal(requests[0].url, '/api/longitudinal_mode')
    assert.equal(requests[0].init.cache, 'no-store')
    assert.deepEqual(JSON.parse(requests[1].init.body), {key: mode, expected: snapshot.values, expires_at: 123, acknowledged: true})
  } finally { globalThis.fetch = previous }
})

test('classic locked longitudinal snapshot never dispatches and reports reason', async () => {
  const {context, requests, fetch, snapshot, messages} = setup()
  snapshot.locked = true
  snapshot.reason = 'Safe Mode is active'
  const previous = globalThis.fetch
  try {
    globalThis.fetch = fetch
    await context.activateFavoriteAction(mode)
    assert.deepEqual(requests.map(request => request.url), ['/api/longitudinal_mode'])
    assert.deepEqual(messages[0], ['Safe Mode is active', 'error'])
  } finally { globalThis.fetch = previous }
})

test('classic recovery accepts Launch Boost metadata and preserves it on repeated readback', async () => {
  const categories = {acceleration: {preset: 'standard', curve: []}, braking: {preset: 'standard', curve: []}, following: {preset: 'medium', curve: []}}
  const profiles = Object.fromEntries(['traffic', 'aggressive', 'standard', 'relaxed'].map(profile => [profile, structuredClone(categories)]))
  profiles.standard.launchBoost = 'low'
  const state = {personalityProfiles: structuredClone(profiles), values: {IsOnroad: false}}
  const context = vm.createContext({state, personalityViewGeneration: 0,
    PERSONALITY_CATEGORY_DEFINITIONS: {acceleration: {}, braking: {}, following: {}},
    window: {location: {pathname: '/device_settings'}}, showParamSnackbar() {},
    fetch: async url => ({ok: true, json: async () => url.includes('personality') ? {profiles} : {IsOnroad: false, IsOffroad: true}}),
  })
  vm.runInContext(extract('async function recoverPersonalitySave()', 'async function savePersonalityCategory('), context)
  for (let retry = 0; retry < 2; retry++) {
    assert.equal(await context.recoverPersonalitySave(), true)
    assert.equal(state.personalityProfilesError, '')
    assert.equal(state.personalityProfiles.standard.launchBoost, 'low')
  }
  profiles.standard.braking = {preset: 'custom', curve: ['invalid']}
  assert.equal(await context.recoverPersonalitySave(), false)
  assert.match(state.personalityProfilesError, /malformed/)
})
