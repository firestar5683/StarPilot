...
@pytest.mark.parametrize('state', [
  {'SafeMode': True}, {'SafeMode': False, 'SafeModeBackup': {}}, {'SafeMode': None},
])
def test_full_restore_refuses_safe_mode_or_pending_restoration(monkeypatch, state):
  from test_backup_api import setup
  from test_personality_profiles_api import the_galaxy as server
  client, _, captured = setup(monkeypatch)
  data = json.loads(client.post('/api/backup').data)
  original = server._safe_params_get_live_raw
  monkeypatch.setattr(server, '_safe_params_get_live_raw', lambda key: state[key] if key in state else original(key))
  response = client.post('/api/restore', json=data)
  assert response.status_code == 400
  assert not captured


def test_full_restore_rechecks_safe_mode_inside_apply(monkeypatch):
  from test_backup_api import setup
  from test_personality_profiles_api import the_galaxy as server
  client, _, _ = setup(monkeypatch)
  data = json.loads(client.post('/api/backup').data)
  original = server._safe_params_get_live_raw
  def apply(*args):
    monkeypatch.setattr(server, '_safe_params_get_live_raw', lambda key: True if key == 'SafeMode' else original(key))
    args[-1]()
    return {'restoredCount': 0, 'addedDrives': 0, 'existingDrives': 0}
