...
@pytest.mark.parametrize("state",[{"SafeMode":True},{"SafeMode":None},{"IsOnroad":None},{"IsOnroad":True,"IsOffroad":True},{"IsOnroad":False,"IsOffroad":False}])
def test_unknown_or_safe_mode_rejects_without_writes(monkeypatch,state):
...
def test_onroad_edit_uses_existing_personality_authoring_policy(monkeypatch):
  ...
  assert client.put(URL,json={"profile":"traffic","level":"low","expected":"high"}).status_code==200
...
def test_road_state_rechecked_after_writer_lock(monkeypatch):
  client,params=_client(monkeypatch)
  class ChangeState:
    def __enter__(self):params.values["SafeMode"]=True
