...
from openpilot.starpilot.system.the_galaxy import cpu_capture  # noqa: E402
...
def test_drive_marker_written_and_excluded_from_row_count(tmp_path):
  ...
  lines = path.read_text().splitlines()
  assert lines[0] == cpu_capture.CSV_HEADER
  assert lines[1].startswith(cpu_capture.DRIVE_MARKER_PREFIX)
  assert "2026-09-04T12:00:00" in lines[1]

  # Marker line is a comment, so it is not counted as a data row.
  assert cpu_capture.capture_status(path)["rows"] == 2

  # Marker round-trips as a skippable comment for CSV readers.
  import csv
  with open(path, newline="") as f:
    data_rows = [row for row in csv.reader(f) if row and not row[0].startswith("#")]
