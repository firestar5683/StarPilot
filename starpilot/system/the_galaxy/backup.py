...
from openpilot.starpilot.common.model_stats import COUNTERS, parse_identity
...
def validate_statistics(data):
  ...
  for row in drives:
    ...
    if state.get('sequence') != row['sequence'] or state.get('gaps') != row['gaps'] or not isinstance(state.get('metrics'), list):
  ...
  for row in metrics:
    ...
    if row['drive'] not in ids or parse_identity(row['owner']) is None or row['mode'] not in ('full', 'aol', 'manual'):
      raise ValueError('Invalid metric ownership')
    pair = (row['drive'], row['owner'], row['mode'])
    if pair in pairs or any(not _number(row[k], not k.endswith('Meters')) for k in COUNTERS):
      raise ValueError('Duplicate or invalid metric counters')
    pairs.add(pair)
  return data
