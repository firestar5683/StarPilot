...
export const PersonalityProfiles = {
  ...
  methods: {
    ...
    async write(action, check = () => !this.editingLocked) {
      // Routine reads must not dim the controls. A click waits for fresh
      // context, then rechecks the same guard before it can send any write.
    ...
    async preset(profile, category, preset) {
      ...
      // Let the backend initialise Custom from the effective current curve,
      // including saved legacy values above the new-point authoring limit.
    ...
    point(profile, category, index, event, preview = false) {
      ...
      curve[index] = value // Preserve untouched legacy points, even above authoring bounds.
    ...
    async setAdvanced(param, raw) {
      // Do not disable a focused number field for routine polling: that can lose
      // its change event. Finish the read first, then use the unchanged write guard.
      if (this.contextPending) { try { await this.contextRequest } catch { return } }
      if (this.disposed) return
      const value = Number(raw)
