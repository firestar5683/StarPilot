...
export const GalaxySelect = {
  ...
  methods: {
    sync() {
      ...
      const items = [...native.options].map((option, index) => ({ index, value: option.value, label: option.label, disabled: option.disabled || (option.parentElement?.tagName === "OPTGROUP" && option.parentElement.disabled), group: option.parentElement?.tagName === "OPTGROUP" ? option.parentElement.label : "" }))
      if (JSON.stringify(items) !== JSON.stringify(this.items)) this.items = items
      this.selected = native.value
      this.label = native.selectedOptions[0]?.label || "Choose an option"
        ...
        <div :id="uid + '-list'" role="listbox" :aria-label="name">
          <template v-for="(item, index) in items" :key="item.index">
            ...
            <button type="button" role="option" :data-value="item.value" :aria-selected="item.value === selected" :disabled="item.disabled" @click="select(item)">
              <span>{{ item.label }}</span><i v-if="item.value === selected" class="bi bi-check-lg" aria-hidden="true"></i>
            </button>
          </template>
        </div>
      </dialog>
