...
export const RoadScore = {
  ...
  methods: {
    ...
    async start(test = false, refine = false) {
      this.busy = true; this.error = ''
      try {
        const samples = []
        for (let i = 0; i < 7; i++) {
        ...
        const value = await this.quick(test ? 'test' : refine ? 'calibration_refine' : 'calibration_start', {attended: true})
        this.calibrationStage = refine ? 'rhythm' : 'markers'
        if (!test && !refine) this.coarseResult = null
    ...
    async pollClicks() {
      ...
      try {
        ...
        if (performance.now() - this.sessionStarted > (this.testMode ? 14000 : this.calibrationStage === 'rhythm' ? 25000 : 55000)) {
          if (!this.testMode && this.taps >= (this.calibrationStage === 'rhythm' ? 8 : this.targetTaps)) await this.finish()
          else { await this.cancel(); if (!this.testMode) this.error = 'Not enough taps. Start this step again and follow the audio.' }
        }
    ...
    async finish() {
      ...
      if (this.result) {
        if (this.calibrationStage === 'markers') this.coarseResult = this.result
        else this.latency = this.result.latency_ms
      }
  ...
  template: `
    <div class="gx-view">
      ...
      <GalaxySection title="Timing correction" icon="bi-clock" :collapsible="false"><div style="padding:16px">
        ...
        <p class="gx-row__desc">Step 1: after two count-in bars, tap 12 spaced chimes to locate the delay. Step 2: tap along with a steady metronome to refine it. Use the screen or Space. Then save and check visual/audio alignment with the three-bar test.</p>
        <button v-if="!session" class="gx-btn" :disabled="busy || !status.can_calibrate" @click="start(false)">Step 1 · Tap the chimes</button>
        <button v-if="!session && coarseResult" class="gx-btn" :disabled="busy || !status.can_calibrate" @click="start(false, true)">Step 2 · Tap with the metronome</button>
        ...
        <p v-if="result && calibrationStage === 'rhythm'">Suggested correction: {{result.latency_ms}} ms · {{result.accepted_taps}} accepted taps · {{result.spread_ms}} ms spread. Review and Save above, then Test timing. Rhythmic tapping can still have a small timing bias.</p>
