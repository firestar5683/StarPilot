2026-09-19 standalone recovery: six current-planner roles are prepared and validated (16.1 MB, no PCM). Main includes standalone local conditioning, corrected Preparing placement/keep-awake, exact archived Mac replay, and conservative-v2 presentation with per-passage beat assessment. The comma connection recovered; the hardware owner restored native replay, staged both local favorites and the plan bank, verified 100 W, and launched the first standalone fresh run. Native acceptance is still in progress; no completed fresh replay or warm-start improvement is claimed. Protected gold and fallback remain unchanged.
