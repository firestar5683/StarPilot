...
for kind,start,seconds,source in [('curve',110,100,'demo'),('arrival',170,87,'arrival')]:
 ...
 if not out.exists():
  subprocess.run([shutil.which('ffmpeg') or '/opt/homebrew/bin/ffmpeg','-v','error','-i',str(R/'assets'/f'{source}.mp4'),'-itsoffset',str(offset),'-i',str(run/'heard.wav'),'-map','0:v:0','-map','1:a:0','-c:v','copy','-c:a','aac','-b:a','192k','-t',str(seconds),str(out)],check=True)
