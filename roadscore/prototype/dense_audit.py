...
out={}
for name in ['stress_baseline','live_curve','live_arrival','other_route','live_development']:
 p=O/name
 ...
 for fname in ['dry','heard']:
  ...
  cutoff=round(json.loads((p/'ending.json').read_text())['audio_frame']/4800) if (p/'ending.json').exists() else len(e)
  measured[fname]={'full_scale_channel_samples':int(np.sum((x>=32767/32768)|(x<=-1))),'peak':float(np.abs(x).max()),'minimum_rms_db_before_ending':float(20*np.log10(max(float(e[:cutoff].min()),1e-9))),'seconds_below_minus50_before_ending':float(sum(e[:cutoff]<10**(-50/20))/10)}
 if name=='live_curve':
  ...
  first_state=next(r for r in rows if r['activation'] is not None)
  event=next(r for r in rows if r['phase']=='event')
  measured['activation_route_t']=first_state['activation'];measured['steering_onset_route_t']=141.531554781;measured['predicted_peak_route_t']=event['predicted_peak'];measured['musical_response_audio_s']=first['audio_s'];measured['physical_apex']='not measured'
  boundaries=[json.loads(t)['new_material_audio_s'] for t in (p/'boundaries.jsonl').read_text().splitlines()]
  measured['generation_boundaries_near_response']=[t for t in boundaries if abs(t-first['audio_s'])<10]
 if (p/'ending.json').exists():
  measured['ending']=json.loads((p/'ending.json').read_text())
  end_s=measured['ending']['audio_frame']/48000
  b=next(b for b in blocks if b['audio_s']<=end_s<b['audio_s']+.100001)
  measured['ending']['rendered_resolution_route_t']=b['callback_wall']-b['replay_origin_wall']+end_s-b['audio_s']
  measured['ending']['estimated_dac_resolution_route_t']=measured['ending']['rendered_resolution_route_t']+b['dac_delay']
  measured['ending']['estimated_dac_silence_route_t']=measured['ending']['estimated_dac_resolution_route_t']+5
  requests=[json.loads(t) for t in (p/'jobs.jsonl').read_text().splitlines()]
  measured['ending']['closing_requests']=[{'route_t':r['route_t'],'play_at_audio_s':r['play_at_audio_s'],'nav':r['nav']} for r in requests if r['conditioning']=='closing']
 out[name]=measured
