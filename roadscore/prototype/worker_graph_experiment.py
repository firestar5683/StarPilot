...
cache_mode=os.environ.get('ROADSCORE_GRAPH_CACHE','')
if cache_mode!='load':cond=tensor(np.concatenate([enc,g[:,None]],axis=1));glob=tensor(g);local=tensor(np.zeros((1,n,257),np.float16))
startup['imports_inputs_seconds']=time.monotonic()-BOOT
if cache_mode=='load':
 from graph_cache import load
 t=time.monotonic();dit,decoder,cond,glob,local,cached_tfs=load();startup['graph_restore_seconds']=time.monotonic()-t
else:
...
tfs=cached_tfs if cache_mode=='load' else [tensor(fourier([t])) for t in sigmas[:-1]]
def generate(req):
...
fast_restore=cache_mode=='load' and os.environ.get('ROADSCORE_GRAPH_FAST')=='1'
if fast_restore:
 from graph_cache import restore_warm
 cfg=json.loads((ROOT/'runtime.json').read_text());r=restore_warm(cfg.get('identity','legacy'))
 t=time.monotonic();dit(tensor(np.load(r['latents'])),tfs[0],cond,glob,local);Device['AMD'].synchronize();startup['graph_link_seconds']=time.monotonic()-t
 startup['reused_startup_audio']=True
else:
 for i in [-1]:
...
if fast_restore:startup['cached_startup_buffer']={k:r.get(k) for k in ['identity','seed','new_seconds','reused_startup_audio']}
else:startup['first_generation']=r
...
if cache_mode=='save':
 from graph_cache import save,stage_warm
 stage_warm(r)
 t=time.monotonic();save((dit,decoder,cond,glob,local,tfs));startup['graph_save_seconds']=time.monotonic()-t
startup['cache_mode']=cache_mode
...
if os.environ.get('ROADSCORE_PROFILE_EXIT')=='1':raise SystemExit(0)
while True:
