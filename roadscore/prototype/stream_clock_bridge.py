...
class StreamClockBridge:
  ...
  def from_callbacks(cls, observations):
    ...
    for wall,current,dac in observations:
      ...
      # DAC timestamps are device-delay estimates and may move a few ms during
      # Bluetooth queue priming. The offset uses the advancing current clock.
      if previous is not None and (wall<previous[0] or current<previous[1]):
