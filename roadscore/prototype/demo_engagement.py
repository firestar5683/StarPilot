...
SIGNAL_MODES = ('recorded', 'left', 'right', 'off')
...
class DemoEngagement:
 def __init__(self, run, session_id, input_mode, *, clock=time.monotonic):
  ...
  self.input_mode=input_mode;self.clock=clock;self.started=clock();self.selection=('recorded','recorded')

 @property
 def mode(self):return self.selection[0]

 @property
 def signal_mode(self):return self.selection[1]
 ...
 def poll(self):
  ...
  try:
   ...
   if (value.get('version')==1 and value.get('session_id')==self.session_id
       ...
       and value.get('signal_mode','recorded') in SIGNAL_MODES
       and math.isfinite(stamp) and self.started<=stamp<=self.clock()):
    selection=(value['mode'],value.get('signal_mode','recorded'))
...
def presentation_signal(mode, recorded_signal):
 return recorded_signal if mode=='recorded' else mode in ('left','right')


def controls_snapshot(mode, signal_mode):
 return {'mode':mode,'signal_mode':signal_mode,'simulated':mode!='recorded' or signal_mode!='recorded'}
