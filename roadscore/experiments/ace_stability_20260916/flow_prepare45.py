...
for role in roles:
 for declared in [120]:
  name=f'window45_{role}';p=OUT/'cases'/name;p.mkdir(parents=True,exist_ok=True);o=p/'official';o.mkdir(exist_ok=True);seed=55100+list(roles).index(role);random.seed(seed);np.random.seed(seed);torch.manual_seed(seed);mx.random.seed(seed)
