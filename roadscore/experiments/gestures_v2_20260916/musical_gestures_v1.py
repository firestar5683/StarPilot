...
class MusicalGestures:
 def __init__(self,source,rate=48000,bpm=128,origin=0.):
  ...
  self.bank=GestureBank(source,rate,bpm);self.phrases={k:self.bank.phrase(k)[0] for k in ['turn_signal','curve_prepare','curve_apex','navigation_turn','lane_change','stop','resume','arrival_prepare','arrival']}
 ...
 def update(self,frame,*,left=False,right=False,road=None,nav=None,speed=0.,outro=False,arrived=False,input_ns=0):
  ...
  with self.lock:
   ...
   if signal and not self.signal:self.signal_next=self.beat(frame+1,4)
   ...
   if signal and not arrived:
    while self.signal_next<frame+self.period:
     self.schedule('turn_signal',self.signal_next,frame,input_ns,'delivered turn signal','signal');self.signal_next+=round(self.period)
   ...
   moving=speed>3
   if self.last_moving is not None and moving!=self.last_moving and frame-self.last_motion>16*self.rate and not outro:
