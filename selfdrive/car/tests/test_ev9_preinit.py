...
def test_controls_update_reuses_early_interface_initialization():
  from openpilot.selfdrive.car.card import Car

  initialized = []
  card_instance = Car.__new__(Car)
  card_instance.initialized_prev = False
  card_instance._initialize_car_interface = lambda: initialized.append(True)
  card_instance.sm = SimpleNamespace(all_alive=lambda _services: False)

  card_instance.controls_update(SimpleNamespace(), SimpleNamespace())
...
def test_stale_cached_ev9_starpilot_safety_is_canonicalized():
  ...
  assert normalized.safetyConfigs[0].safetyParam == 0x8C95
