...
def test_ev9_long_preinit_firmware_selection():
  assert EV9_LONG_PREINIT_FIRMWARE_NAMES == (
...
def test_ignore_ignition_line_keeps_existing_platform_independent_behavior():
  assert get_ignore_ignition_line(FakeParams({"CarMake": "gm", "IgnoreIgnitionLine": True}))
  assert get_ignore_ignition_line(FakeParams({"CarMake": "hyundai", "IgnoreIgnitionLine": True}))
